...
def _run_embedding(
    ...
    # Forward latency is per batch; VIT_EMBEDDING_RT_METRIC in submit_and_wait
    # includes the per-request queue and batch-collection wait as well.
...
class MMScheduler:
    ...
    def __init__(
        ...
        forward_profiler: Optional[Callable] = None,
    ):
        if batch_wait_ms < 0:
    ...
    @property
    def max_request_images(self):
        import sys

        return sys.maxsize if self._work_budget is not None else self._max_batch_images
    ...
    def submit_and_wait(self, work_items: List[MMWorkItem]) -> None:
        ...
        with self._lock:
            ...
            if len(self._admitted_requests) >= self._max_queue_size + 1:
                kmonitor.report(AccMetrics.VIT_EMBEDDING_OVERLOAD_QPS_METRIC, 1)
                raise MMSchedulerOverloadError(
                    "MMScheduler queue full, request rejected"
                )
            self._admitted_requests.add(req)
        ...
        try:
            ...
            with self._lock:
                req.cancelled = True
                req.future.cancel()
                self._admitted_requests.discard(req)
    ...
    def _fail_chunks(self, chunks, error):
        for chunk in chunks:
            request = chunk.request
            if request.future.done():
                continue
            request.cancelled = True
            failure = MMSchedulerExecutionError(
                type(error).__name__,
                str(error),
                isinstance(error, torch.cuda.OutOfMemoryError),
            )
            with self._lock:
                self._admitted_requests.discard(request)
                if not request.future.done():
                    request.future.set_exception(failure)
            request.done.set()

    def _complete_chunk(self, chunk):
        request = chunk.request
        with self._lock:
            ...
            if self._stopped.is_set():
                ...
                request.future.set_exception(
                    MMSchedulerExecutionError(
                        "RuntimeError", "MMScheduler closed before request completed"
                    )
                )
                request.done.set()
                return
        ...
        try:
            while not self._stopped.is_set():
                ...
                try:
                    ...
                    logging.exception("MMScheduler executor loop failed")
                    self._clear_exception_references(error)
    ...
    def _claim_batch(self, batch):
        with self._lock:
            ...
            return [
                chunk
                for chunk in batch
                if not chunk.request.cancelled
                and (
                    chunk.request.future.running()
                    or chunk.request.future.set_running_or_notify_cancel()
                )
            ]

    def _fail_all_queued(self):
        queued = [self._pending] if self._pending else []
        self._pending = None
        queued.extend(self._drain(self._waiting))
        self._fail_chunks(
            queued, RuntimeError("MMScheduler closed before request completed")
        )
        self._report_queue_depth()
    ...
    def _run_items_with_oom_split(self, items: List[MMWorkItem]) -> None:
        ...
        try:
            profiler = (
                self._forward_profiler() if self._forward_profiler else nullcontext()
            )
            with profiler:
    ...
    def _execute_batch(self, batch: List[_EmbeddingChunk]) -> None:
        ...
        # Callers may time out during the failed forward. Recheck on every
        # recursive retry, not only when the scheduler initially claims work.
        ...
        kmonitor.report(GaugeMetrics.VIT_EMBEDDING_BATCH_SIZE_METRIC, len(batch))
        for chunk in batch:
            self._complete_chunk(chunk)
    ...
    def close(self, timeout: float = 10.0) -> bool:
        ...
        with self._lock:
            if self._stopped.is_set() and not self._executor.is_alive():
                return True
            self._stopped.set()
