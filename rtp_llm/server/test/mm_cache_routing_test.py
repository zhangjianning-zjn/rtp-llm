...
class MMCacheRoutingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stale_vit_reselection_acquires_new_hashes_before_prefill(self):
        ...
        self.assertIsNone(await self.visitor.get_master_route_addrs(self.request))
        calls = self.visitor.master_client.get_backend_role_addrs.call_args_list
        self.assertTrue(calls[2].kwargs["vit_only"])
        self.assertNotIn("input_pb", calls[2].kwargs)
        self.assertIn("input_pb", calls[3].kwargs)
        last = calls[-1].kwargs
    ...
    async def test_short_http_timeout_does_not_shorten_pending_placement_ttl(self):
        ...
        self.assertEqual(call.args[2], 0.5)
        self.assertTrue(call.args[1].vit_only)
        self.assertFalse(call.args[1].generate_input)
...
class MMCacheApiTest(unittest.TestCase):
    ...
    def test_rdma_response_keeps_inline_feature_hashes(self):
        ...
        server._transport = Mock()
        server._transport.transfer.return_value = MultimodalOutputPB(
            output_rdma_slots=[
                MMRdmaSlotPB(rdma_descriptor=RdmaDescriptorPB(lease_id="handle"))
            ],
            split_size=[2],
        )
        ...
        server.engine = Mock()
        server.engine.get_embedding_result.return_value = [result]
        response = server.RemoteMultimodalEmbedding(MultimodalInputsPB(), Mock())
        self.assertEqual(
