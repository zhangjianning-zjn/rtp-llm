...
def rocm_oss_suites():
    ...
    native.test_suite(
        ...
        tests = [
            ...
            smoke_test(
                # page=32 exceeds the FP8 width (16), so the vectorized view differs
                # from the linear one and the request spans more than one block:
                # cross-block vectorized indexing is actually exercised here.
                name="rocm_dense_qwen3_8b_ptpc_fp8kv_triton_vectorized",
                task_info="data/model/qwen3/ptpc_q_r_8b.json",
                smoke_args="--quantization FP8_PER_CHANNEL_COMPRESSED --use_swizzleA 1 --use_asm_pa 0 --use_aiter_pa 1 --use_triton_pa 1 --disable_flashinfer_native 1 --fp8_kv_cache 1 --seq_size_per_block 32 --kernel_seq_size_per_block 32 --enable_cuda_graph 1 --warm_up 1 --act_type BF16 --reserver_runtime_mem_mb 70000 --test_block_num 64",
                gpu_type=["MI308X-ROCM7"],
            ),
            smoke_test(
