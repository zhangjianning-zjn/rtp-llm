...
ErrorResult<MultimodalOutput> MultimodalPbConverter::inlineOutputFromPb(const MultimodalOutputPB& output_pb) {
    ...
    try {
        ...
        const auto hash_status = attachFeatureHashes(output_pb, mm_output);
        if (!hash_status.ok()) {
            return hash_status;
        }
        return mm_output;
...
ErrorInfo MultimodalPbConverter::attachFeatureHashes(const MultimodalOutputPB& output_pb, MultimodalOutput& output) {
    if (!output_pb.has_multimodal_feature_hash()) {
        return ErrorInfo::OkStatus();
    }
    auto                 hashes = TensorPbConvert::pbToTorch(output_pb.multimodal_feature_hash());
    std::vector<int64_t> sizes(output_pb.split_size().begin(), output_pb.split_size().end());
    ...
    output.mm_feature_hashes = hashes.split(sizes, 0);
    return ErrorInfo::OkStatus();
}
