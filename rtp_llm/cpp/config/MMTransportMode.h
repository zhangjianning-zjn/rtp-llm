...
inline std::string validateMMTransportMode(std::string mode) {
    if (mode != kMMTransportModeAuto && mode != kMMTransportModeGrpc && mode != kMMTransportModeRdma) {
        throw std::invalid_argument("invalid mm transport mode '" + mode + "'; expected 'auto', 'grpc' or 'rdma'");
    }
    return mode;
}
